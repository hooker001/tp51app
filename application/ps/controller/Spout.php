...
use app\ps\validate\Spout as Vd;
use app\ps\model\Spout as Mdl;
use think\Db;

class Spout extends Controller
{
    protected $fcode = '060208';
    ...
    public function create()
    {
        ...
        $usid .= $strId;
        $sql = "update ps_spout_zy set geom=st_geomfromgeojson('$geom'),usid='$usid' where gid=" . $id;
        Db::execute($sql);
        return jsonSuc();
    }
    ...
    public function update()
    {
        ...
        if ($geom) {
            $sql = "update ps_spout_zy set geom=st_geomfromgeojson('$geom') where gid=" . $id;
            Db::execute($sql);
        }
        return jsonSuc();
    }
